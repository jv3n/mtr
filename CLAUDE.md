...
- **Gestion de position : Double Top** (`docs/pattern/dt.md`, #16) — s'applique quel que soit le déclencheur d'entrée. **Stop à 11 %** (du cas TRIB : entrée 3,14 $ / stop 3,49 $), pas 5 % : la largeur sert à absorber un dépassement mineur du sommet. ⚠️ `dt.md` mentionne aussi « risque ~25 % de la position » — c'est le seul chiffre sans exemple chiffré derrière, dans une transcription que le doc dit lui-même fragmentaire, donc lu comme une tolérance après slippage, pas comme la distance de stop. **Le sizing découle du risque** (`riskPerTradeUsd / stopLossPct`, plafonné par `maxNotionalUsd`) et non d'un notionnel fixe : élargir le stop ne doit pas multiplier en douce la perte max par trade face à `maxDailyLossUsd`. **Trailing stop** armé seulement après 5 % de gain (la leçon interdit de resserrer trop tôt). **`ENTRY_CUTOFF` (15h NY) ≠ `SESSION_END`** : arrêter les entrées et solder le book sont deux moments distincts — les confondre était le bug signalé par #16.
- **SSR (Reg SHO Rule 201)** : ⚠️ **ni TradeZero ni Massive ne l'exposent** (vérifié sur les deux catalogues d'API le 2026-07-23 ; TZ ne donne qu'`is-easy-to-borrow` et, indirectement via les types de locate, la liste *threshold* Reg SHO — **une autre règle**, les fails-to-deliver, pas le coupe-circuit à −10 %). Donc **dérivé** de la règle : SSR actif dès qu'un titre touche ≤ 90 % de la clôture veille, valable ce jour-là **+ tout le lendemain**. C'est le cas **hérité de la veille** qui compte ici : notre univers gappe +50 %, donc déclencher en intraday supposerait de rendre tout le gap. Récupéré via un appel groupé de bars journalières (`/v2/aggs/grouped/...`), en remontant jusqu'à trouver 2 séances. **Un titre en SSR se trade quand même** — c'est un red flag qui coûte de la taille (`ssrSizeMultiplier`), pas un veto. **À revalider marché ouvert.**
...
- **Dependabot** (`.github/dependabot.yml`) : PR hebdo (lundi 7h, Europe/Paris) sur 3 écosystèmes — `gradle` (deps, versions de plugins **et le wrapper**), `github-actions`, `docker` (images de base du `Dockerfile` + `docker-compose.yml`). Mineures/patch **groupées** en une PR par écosystème, **majeures séparées** (elles demandent une décision : JDK 26, Ktor 4, Kotlin 3…). Préfixes de commit forcés (`build`/`ci`) pour rester en Conventional Commits. Vérifié au premier passage : la version Ktor portée par `val ktor = "..."` est bien résolue, et le wrapper Gradle est bumpé (`gradlew`, `gradlew.bat`, le jar et `gradle-wrapper.properties`) — le mode `100755` de `gradlew` est préservé.
  - ⚠️ **Relire les PR qui touchent au wrapper** : régénérer `gradlew.bat` réécrit ses fins de ligne (diff « fichier entier »). Le `.gitattributes` (`* text=auto eol=lf`) doit rester la référence — cf. le piège « Fins de ligne » ci-dessus.
